...
def mat_time_cal(data_index_path_base, mat_log_file_base, start_num, end_num, step_num, data_num, re_calculate=False):
    ...
    mat_time_history = np.zeros((int((end_num - start_num) / step_num)+1, data_num*60))
    ...
    if re_calculate:
        for num_publisher in range(start_num, end_num+1, step_num):
            ...
            for data_idx in range(data_num):
                print(f"data_idx = {data_idx} start")
                data_index_path = data_index_path_base + str(num_publisher) + "_fix" + str(num_publisher) + "_" + str(data_idx) +  ".csv"
                mat_log_file = mat_log_file_base + str(num_publisher) + "_" + str(data_idx) + ".csv"

                with open(mat_log_file, "w") as f:
                    pass
                ...
                for time in range(0, simulation_time, time_step):
                    ...
                    values_batch, actions_batch, action_log_probs_batch, mat_time = runner.collect(trainer, buffer, step, deterministic=False)

                    mat_time_history[int(num_publisher / step_num)-1][data_idx*60 + step] = mat_time

                    with open(mat_log_file, "a") as f:
                        f.write(f"{time},{mat_time}\n")
            ...
            for data_idx in range(data_num):
                ...
                with open(mat_log_file, "r") as f:
                    ...
                    for time in range(0, simulation_time, time_step):
                        ...
                        mat_time_history[int(num_publisher / step_num)-1][data_idx*60 + step] = float(line[1])

    return mat_time_history


def RELOC_time_cal(data_index_path_base, reloc_log_file_base, start_num, end_num, step_num, data_num, K=3, re_calculate=False):
    print(f"RELOC_time_cal start")
    reloc_time_history = np.zeros((int((end_num - start_num) / step_num)+1, data_num*60))
    ...
    if re_calculate:
        for num_publisher in range(start_num, end_num+1, step_num):
            ...
            for data_idx in range(data_num):
                print(f"data_idx = {data_idx} start")
                data_index_path = data_index_path_base + str(num_publisher) + "_fix" + str(num_publisher) + "_" + str(data_idx) +  ".csv"
                reloc_log_file = reloc_log_file_base + str(num_publisher) + "_" + str(data_idx) + ".csv"

                with open(reloc_log_file, "w") as f:
                    pass
                ...
                for time in range(0, env.simulation_time, env.time_step):
                    ...
                    reloc_time = reloc_end - reloc_start

                    reloc_time_history[int(num_publisher / step_num)-1][data_idx*60 + step] = reloc_time

                    with open(reloc_log_file, "a") as f:
                        f.write(f"{time},{reloc_time}\n")
        ...
        for num_publisher in range(start_num, end_num+1, step_num):
            ...
            for data_idx in range(data_num):
                ...
                with open(reloc_log_file, "r") as f:
                    f = list(f)
                    for time in range(0, env.simulation_time, env.time_step):
                        step = int(time/env.time_step)
                        line = f[step]
                        ...
                        reloc_time_history[int(num_publisher / step_num)-1][data_idx*60 + step] = float(line[1])

    return reloc_time_history


data_index_path_base = "../dataset/master_thesis/single_data/index/mat_time_client"
mat_log_file_base = "../result/save/master_thesis/mat_time/mat_time_client"
reloc_log_file_base = "../result/save/master_thesis/mat_time/reloc_time_client"
result_fig_path = "../result/save/master_thesis/mat_time/mat_time.pdf"
...
mat_time_history = mat_time_cal(data_index_path_base, mat_log_file_base, start_num, end_num, step_num, data_num, re_calculate=re_calculate)
reloc_time_history = RELOC_time_cal(data_index_path_base, reloc_log_file_base, start_num, end_num, step_num, data_num, K=3, re_calculate=re_calculate)

mat_time_ave = np.mean(mat_time_history, axis=1).reshape(-1)
reloc_time_ave = np.mean(reloc_time_history, axis=1).reshape(-1)

mat_std = np.std(mat_time_history, axis=1).reshape(-1)
reloc_std = np.std(reloc_time_history, axis=1).reshape(-1)

width = 3.5

mat_x = np.arange(start_num, end_num+1, step_num) - width/2
mat_y = np.array(mat_time_ave)

reloc_x = np.arange(start_num, end_num+1, step_num) + width/2
reloc_y = np.array(reloc_time_ave)
...
wind.set_ylabel("average solving time (s)")
wind.set_xticks(labels)
wind.bar(mat_x, mat_y, yerr=mat_std, capsize=5, width=width, label="Proposed")
wind.bar(reloc_x, reloc_y, yerr=reloc_std, capsize=5, width=width, label="RELOC")
wind.legend()
