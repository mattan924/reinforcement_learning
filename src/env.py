...
class Env:
    ...
    def get_observation_mat(self, agent_perm, topic_perm, obs_size=81, debug=False):
        ...
        #  0~728    : あるトピックの publisher の分布
        #  729~14557: あるトピックの subscriber の分布
        #  1458~2186: クライアントの分布
        #  2187~2188: クライアントの位置 (x, y)
        ...
        for i in range(self.num_client):
            ...
            position_info_client[i][0] = client.x
            position_info_client[i][1] = client.y
        ...
        for edge in self.all_edge:
            ...
            for t in range(self.num_topic):
                topic_id = topic_perm[t]
                topic_storage_info[t][edge_id] = edge.used_volume[topic_id]
                topic_cpu_used_client[t][edge_id] = edge.used_publishers[topic_id]


        obs[:, :, 0:channel_dim] = publisher_distribution[np.newaxis]
        obs[:, :, channel_dim:channel_dim*2] = subscriber_distribution[np.newaxis]
        obs[:, :, channel_dim*2:channel_dim*3] = total_distribution[np.newaxis, np.newaxis]
        obs[:, :, channel_dim*3:channel_dim*3+2] = position_info_client[:, np.newaxis]
        ...
        for t in range(self.num_topic):
            ...
            obs[:, t, -3] = topic.require_cycle * 1e5
            obs[:, t, -2] = topic.data_size * 1e5
            obs[:, t, -1] = topic.volume * 1e2
